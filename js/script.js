...
function autoFillCheckoutForm() {
    ...
    if (user) {
        ...
        // No WhatsApp diisi dari data tersimpan di /users/{uid}/phone (jika ada)
        const savedPhone = user.dbData && user.dbData.phone ? user.dbData.phone : '';
        if (savedPhone && (!phoneInput.value || phoneInput.dataset.autofilled === 'true')) {
        ...
        phoneInput.addEventListener('change', function () {
            ...
            if (window.currentUser && window.currentUser.uid && typeof firebase !== 'undefined') {
                firebase.database().ref('users/' + window.currentUser.uid + '/phone').set(phoneInput.value);
            }
        });
