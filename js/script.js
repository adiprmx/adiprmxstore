...
function renderProducts() {
    ...
    if (flmGrid) {
        if (flmFiltered.length > 0) {
            flmGrid.innerHTML = flmFiltered.map((product, index) => `
            ...
            `).join('');
            flmGrid.style.display = 'grid';
        } else {
            flmGrid.innerHTML = '';
            flmGrid.style.display = 'none';
        }
    }
    
    // Render Sample Pack Products (tanpa tombol play)
    const sampleGrid = document.getElementById('sampleGrid');
    if (sampleGrid) {
        if (sampleFiltered.length > 0) {
            // Show only first 12 items initially
            const initialProducts = sampleFiltered.slice(0, 12);
            ...
            if (sampleFiltered.length > 12) {
                ...
                loadMoreBtn.innerHTML = `
                    <button class="btn btn-outline" onclick="loadMoreSamples()">
                        <i class="fas fa-chevron-down"></i> Lihat Lebih Banyak (${sampleFiltered.length - 12} lagi)
                    </button>
                `;
...
function loadMoreSamples() {
    ...
    const sampleFiltered = window.filteredSample || samplePackProducts;
    const nextProducts = sampleFiltered.slice(loadedSamples, loadedSamples + 12);
    ...
    if (loadMoreContainer) {
        const remaining = sampleFiltered.length - loadedSamples;
        if (remaining > 0) {
            loadMoreContainer.querySelector('button').innerHTML = `
                <i class="fas fa-chevron-down"></i> Lihat Lebih Banyak (${remaining} lagi)
            `;
        } else {
